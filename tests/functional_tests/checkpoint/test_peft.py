...
def get_test_hf_peft_checkpoint_expected_keys():
    ...
    In transformers v5, Mixtral MoE experts use a fused MixtralExperts module
    that is not a standard nn.Linear, so LoRA is only applied to:
    - Attention projections (q_proj, k_proj, v_proj, o_proj)
    - lm_head

    Note: MixtralTopKRouter (gate) is also not nn.Linear in v5, so no LoRA there either.
    """
    expected_model_keys = {
        # Layer 0 attention
