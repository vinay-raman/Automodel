...
```bash
pip3 install "nemo-automodel[vlm,vlm-media]"
```
...
```bash
pip3 install "nemo-automodel[vlm,vlm-media]"              # VLM + media
pip3 install "nemo-automodel[diffusion,diffusion-media]"  # diffusion + media
```
...
After `pip3 install nemo-automodel[all]` (or inside the container) add media
explicitly: `pip3 install nemo-automodel[media]`.
...
Install all standard extras (CUDA, VLM, NeMo-Run, diffusion, etc.):
...
<Note>
`[all]` intentionally **excludes the media extras** (FFmpeg-bearing). Add them
explicitly if you need video/image decode:
`pip3 install nemo-automodel[all,media]`.
...
| VLM + media (Qwen/Mistral/video) | `pip3 install "nemo-automodel[vlm,vlm-media]"`             |
| Diffusion + media           | `pip3 install "nemo-automodel[diffusion,diffusion-media]"`      |
