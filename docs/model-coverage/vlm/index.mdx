...
<Note>
VLMs that decode video or use Qwen / Mistral vision preprocessing also need the `vlm-media` extra (`pip install "nemo-automodel[vlm,vlm-media]"`), which is not installed in the Docker container by default. See [Media Extras](/get-started/installation#media-extras-video--image-decode).
</Note>
