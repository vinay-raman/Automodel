...
class Gemma4DSparkAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        k = self._repeat_kv(k)
        v = self._repeat_kv(v)
        if attention_mask is not None and self.config._attn_implementation == "flex_attention":
...
class Gemma4DSparkModel(Gemma4PreTrainedModel):
    ...
    def compute_logits(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if softcap is not None:
            ...
            assert softcap > 0.0, "config.final_logit_softcapping must be positive when provided."
